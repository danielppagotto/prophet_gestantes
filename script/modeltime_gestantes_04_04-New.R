...
nascimentos_previsao_tempos <- read_excel("nascimentos_previsao_tempos.xlsx")


tempos_procedimentos <- nascimentos_previsao_tempos %>% 
  select(batch, ano, regiao, starts_with("tempo_")) %>% 
  group_by(batch, ano, regiao) %>% 
  summarise_all(sum) 
...
tempo_profissionais %>% 
  ...
  geom_col(group = 1) + facet_wrap(~regiao) + 
  theme_minimal()
